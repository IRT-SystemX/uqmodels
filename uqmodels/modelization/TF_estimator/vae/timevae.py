...
class TrendLayer(layers.Layer):
    ...
    def __init__(self, dim_in: int, trend_poly: int, dim_seq: int, **kwargs):
        super().__init__(**kwargs)
        self.dim_in = dim_in
        self.trend_poly = int(trend_poly)
        self.dim_seq = int(dim_seq)
        self.trend_dense1 = layers.Dense(self.dim_in * self.trend_poly, activation="relu", name="trend_params")
        self.trend_dense2 = layers.Dense(self.dim_in * self.trend_poly, name="trend_params2")
        self.reshape_layer = layers.Reshape(target_shape=(self.dim_in, self.trend_poly))
    ...
    def call(self, z):
        ...
        lin_space = tf.range(0.0, float(self.dim_seq), 1.0) / float(self.dim_seq)       # (T,)
...
class SeasonalLayer(layers.Layer):
    ...
    def __init__(self, dim_in: int, dim_seq: int, custom_seas: list[tuple[int, int]], **kwargs):
        super().__init__(**kwargs)
        self.dim_in = dim_in
        self.dim_seq = dim_seq
        self.custom_seas = custom_seas
        self.dense_layers = [layers.Dense(dim_in * s, name=f"season_params_{i}") for i, (s, L) in enumerate(custom_seas)]
        self.reshape_layers = [layers.Reshape(target_shape=(dim_in, s)) for s, L in custom_seas]
    ...
    def _get_season_indexes_over_seq(self, num_seasons, len_per_season):
        ...
        season_idx = tf.tile(season_idx, [self.dim_seq // len_per_season + 1])[: self.dim_seq]  # (T,)
        return season_idx  # (T,)

    def call(self, z):
        N = tf.shape(z)[0]
        ones = tf.ones(shape=[N, self.dim_in, self.dim_seq], dtype=tf.int32)  # (N, D, T)
        all_vals = []
...
def build_time_vae_encoder(dim_seq: int, dim_in: int, dim_z: int, conv_filters: list[int],variational=True) -> tf.keras.Model:
    """
    Encoder (B, T, F) -> (z_mean, z_log_var, z) avec z de shape (B, dim_z).
...
def build_time_vae_decoder(
    ...
    *,
    ...
    level = layers.Dense(dim_in, activation="relu", name="level_params")(dec_in)
    level = layers.Dense(dim_in, name="level_params2")(level)
    level = layers.Reshape((1, dim_in))(level)                # (N,1,F)
    ones_T = tf.ones((1, dim_seq, 1), dtype=tf.float32)
    ...
    if trend_poly and trend_poly > 0:
        trend_vals = TrendLayer(dim_in, trend_poly, dim_seq)(dec_in)  # (N,T,F)
        outputs = outputs + trend_vals

    # --- saisonnalités custom (optionnelles) ---
    if custom_seas and len(custom_seas) > 0:
        seas_vals = SeasonalLayer(dim_in, dim_seq, custom_seas)(dec_in)  # (N,T,F)
    ...
    if use_residual_conn and conv_filters:
        ...
        x = layers.Conv1DTranspose(filters=dim_in, kernel_size=3, strides=2, padding="same",
        ...
        residuals = layers.Reshape((dim_seq, dim_in), name="decoder_reshape_final")(x)
...
class TimeAE(BaseAutoencoder):
    def __init__(self, dim_seq, dim_in, dim_z, conv_filters=(50,100,200),
        ...
        self.encoder = build_time_vae_encoder(dim_seq, dim_in, dim_z, list(conv_filters),variational=False)
        self.decoder = build_time_vae_decoder(
            dim_seq, dim_in, dim_z, list(conv_filters),
...
class TimeVAE(BaseVariationalAutoencoder):
    def __init__(self, dim_seq, dim_in, dim_z, conv_filters=(50,100,200),
        ...
        all_init_params = {**explicit_args, **kwargs}
        super().__init__(**all_init_params)
        self.encoder = build_time_vae_encoder(dim_seq, dim_in, dim_z, list(conv_filters),variational=True)
        self.decoder = build_time_vae_decoder(
            dim_seq, dim_in, dim_z, list(conv_filters),
            trend_poly=trend_poly, custom_seas=custom_seas, use_residual_conn=use_residual_conn
        )