...
# Lancement de flake8
flake8 abench  --config=scripts/.flake8 --format=html --htmldir=_static/flake8 --exit-zero 
...
# Lancement de pylint
pylint abench --rcfile=scripts/.pylintrc --output-format=text --exit-zero | tee _static/pylint/pylint.txt
PYLINT_SCORE=$(sed -n 's/^Your code has been rated at \([-0-9.]*\)\/.*/\1/p' _static/pylint/pylint.txt)
pylint abench --rcfile=scripts/.pylintrc --output-format=pylint_gitlab.GitlabPagesHtmlReporter --exit-zero > _static/pylint/index.html 
anybadge --overwrite --label pylint --value=$PYLINT_SCORE --file=_static/pylint/pylint.svg 4=red 6=orange 8=yellow 10=green

# Ruff check

ruff check abench/ > scripts/ruff_report.txt 
